...
if (fs.existsSync(app_json_path))
    app_config = JSON.parse(fs.readFileSync(app_json_path));
...
config.exclude = app_config.exclude || config.exclude || [];

config.useGivenNames = app_config.useGivenNames || config.useGivenNames || false;

if (app_config.logging)
    app_config.globalLogLevel = app_config.logging.globalLogLevel;
if (config.logging)
    config.globalLogLevel = config.logging.globalLogLevel;

config.globalLogLevel = app_config.globalLogLevel || config.globalLogLevel || 'info';

config.createTempCopyOfInstrumention = app_config.createTempCopyOfInstrumention || config.createTempCopyOfInstrumention || false;

config.methodSignatureSeparator = '&&~\%\%';

config.compressPosts = app_config.compressPosts || config.compressPosts || false;

config.active_debug_service_type = app_config.active_debug_service_type || config.active_debug_service_type || 'file';

config.public_configuration_file_reload();

config.debug_services = app_config.debug_services || config.debug_services;
