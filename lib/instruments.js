...
var instruments = {
    ...
    prepareLogMessage: function(logObject, direction) {
        ...
        if ((logObject.funcText !== 'An anonymous function ') && (logObject.funcText !== 'anonymous_function')) {
            ...
            if (config.logAnonymousFunctions) {
                formattedMessage = formatMessage(logObject.filename,
    ...
    // Loop backwards, from the latest method up the chain
    // In every cycle we need to change the code if we're not in the top most level
    // this way we can "trace" who's calling who
    lookupFunctionCallerCode: function(code) {
        var item = inProcess.current;
        for (var i = 0; i < inProcess.size; i++) {
            if (item.value.caller === code) {
                return item.key;
            }
            if (i > 1) {
                code = item.value.caller;
            }
            item = inProcess.next();
        }
    ...
    handlePreMessage: function(name, argsNames, args, moduleName, methodId, lineNumber, isAnonymous) {
        ...
        // process stack, if so use the caller flowId
        var keyForFlowId = this.lookupFunctionCallerCode(callerFlowId);
        if (keyForFlowId > 0) {
            flowId = inProcess.get(keyForFlowId).value.debugData[0].flowId;
        }
        ...
        method.caller = callerFlowId.toLowerCase() === 'callbacks' ? null : callerFlowId ;
    ...
    handlePostMessage: function(name, result, moduleName, lineNumber, methodId) {
        ...
        if (method !== null) {
            ...
            delete (method.caller); // Remove this property before submitting this to the server

            if (config.postUsingChildProcess) {
                if ((this._childProcess === null) || (this._childProcess === undefined)) {
                    this._childProcess = childProcess.fork('./../node_offline_debug/lib/child_process');
                }
                var sendData = {
                    "start": true,
                    "type": "postData",
                ...
                this._childProcess.send(sendData);

                this._childProcess.on('message', function (msg) {
                    that._finalizedData = msg.content;
                }.bind(this));
