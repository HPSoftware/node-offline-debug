...
var instruments = {
    ...
    loadSourceFile: function () {
        if (config.fileToLoad.length > 0) {
            var filename = config.fileToLoad;
            config.fileToLoad = '';

            fileLoad.load(path.resolve(filename), this.postSourceCode);
        }
    },

    postSourceCode: function (sourceCode) {
        logger.warn(sourceCode);
    },
