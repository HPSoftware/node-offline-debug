  ...
  postBackLog: function (logObject) {
    ...
    var auth = 'Basic ' + new Buffer(config.username + ":" + config.password).toString('base64'),
        ...
        options = {
            "port": 443, // SSL
            "host": config.url,
            "path": "/OfflineDebugger/DebugData/Data/" + config.username,
            "headers": {
                "Authorization": auth,
    ...
    var request = http.request(options, function (response) {
      ...
      response.on('end', function() {
          console.log(JSON.parse(logObjectString).debugData[0].functionName + ', ' + JSON.parse(logObjectString).debugData[0].functionFile);
      });
    });
