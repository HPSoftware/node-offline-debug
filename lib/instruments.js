...
var instruments = {
    ...
    isActive: function() {
        return (config.active === undefined || config.active === true);
    },
    ...
    getFunctionGivenName: function (filename, lineNumber) {
        var func = this.lookupMap.get(filename + '&&~%%' + lineNumber);
    ...
    prepareLogMessage: function(logObject, direction) {
        ...
        if ((logObject.funcText !== 'An anonymous function ') && (logObject.funcText !== 'anonymous_function')) {
            if (that.lookupSet.contains(logObject.filename + '&&~%%' + logObject.lineNumber)) {
